...
impl<M> BoxedLimbs<M> {
    fn positive_minimal_width_from_be_bytes(input: untrusted::Input) -> Result<Self> {
        // Reject leading zeros. Also reject the value zero ([0]) because zero
        // isn't positive.
        if untrusted::Reader::new(input).peek(0) {
            return Err(Error::from(ErrorKind::LimbUnspecifiedError));
        }
    ...
    fn from_be_bytes_padded_less_than(input: untrusted::Input, m: &Modulus<M>) -> Result<Self> {
...
impl<M: PublicModulus> core::fmt::Debug for Modulus<M> {
    fn fmt(&self, fmt: &mut ::core::fmt::Formatter) -> std::result::Result<(), ::core::fmt::Error> {
...
impl<M> Modulus<M> {
    ...
    ) -> Result<(Self, bits::BitLength)> {
        let limbs = BoxedLimbs::positive_minimal_width_from_be_bytes(input)?;
        Self::from_boxed_limbs(limbs)
    }

    pub fn from_nonnegative_with_bit_length(n: Nonnegative) -> Result<(Self, bits::BitLength)> {
    ...
    fn from_boxed_limbs(n: BoxedLimbs<M>) -> Result<(Self, bits::BitLength)> {
...
impl<M> Elem<M, Unencoded> {
    pub fn from_be_bytes_padded(input: untrusted::Input, m: &Modulus<M>) -> Result<Self> {
        Ok(Elem {
    ...
    pub fn into_modulus<MM>(self) -> Result<Modulus<MM>> {
        let (m, _bits) =
...
impl PublicExponent {
    pub fn from_be_bytes(input: untrusted::Input, min_value: u64) -> Result<Self> {
        if input.len() > 5 {
            return Err(Error::from(ErrorKind::CryptoError));
        }
        let value = input.read_all(Error::from(ErrorKind::CryptoError), |input| {
...
impl<M> PrivateExponent<M> {
    pub fn from_be_bytes_padded(input: untrusted::Input, p: &Modulus<M>) -> Result<Self> {
...
#[inline]
pub fn elem_verify_equal_consttime<M, E>(a: &Elem<M, E>, b: &Elem<M, E>) -> Result<()> {
...
impl Nonnegative {
    ...
    pub fn verify_less_than(&self, other: &Self) -> Result<()> {
    ...
    pub fn to_elem<M>(&self, m: &Modulus<M>) -> Result<Elem<M, Unencoded>> {
    ...
    pub fn verify_less_than_modulus<M>(&self, m: &Modulus<M>) -> Result<()> {
