...
impl BitLength {
    ...
    #[inline]
    pub fn from_usize_bytes(bytes: usize) -> Result<Self> {
        let bits = bytes
            .checked_mul(8)
            .ok_or(Error::from(ErrorKind::LimbUnspecifiedError))?;
    ...
    #[cfg(feature = "alloc")]
    #[inline]
    pub fn try_sub_1(self) -> Result<BitLength> {
        let sum = self.0.checked_sub(1).unwrap();
        Ok(BitLength(sum))
    }
}
